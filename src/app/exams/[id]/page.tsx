...
import { FileText, Edit, Download, QrCode, Copy, Share2, Clock, Calendar, Users, BarChart, CheckCircle, XCircle, AlertCircle, ChevronLeft, Send } from 'lucide-react'
...
export default function ExamDetailsPage() {
  ...
  const exam: Exam = {
    ...
    students: [
      { id: "1", name: "Ana Silva", email: "ana.silva@escola.edu", grade: "8º Ano A", score: 6.5, maxScore: 7, submittedAt: "15/03/2025 10:30", status: "completed" },
      { id: "2", name: "Bruno Santos", email: "bruno.santos@escola.edu", grade: "8º Ano A", score: 7, maxScore: 7, submittedAt: "15/03/2025 10:15", status: "completed" },
      { id: "3", name: "Carla Oliveira", email: "carla.oliveira@escola.edu", grade: "8º Ano A", score: 5, maxScore: 7, submittedAt: "15/03/2025 10:45", status: "completed" },
      { id: "4", name: "Daniel Pereira", email: "daniel.pereira@escola.edu", grade: "8º Ano A", status: "in_progress" },
      { id: "5", name: "Eduarda Lima", email: "eduarda.lima@escola.edu", grade: "8º Ano A", status: "pending" },
      { id: "6", name: "Felipe Costa", email: "felipe.costa@escola.edu", grade: "8º Ano A", status: "absent" }
    ],
    averageScore: 6.2,
    completionRate: 50
  };
    ...
    <div className="flex min-h-screen flex-col">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="lg:w-2/3 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Disciplina:</span>
                      <span className="text-sm">{exam.subject}</span>
                    </div>
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Turma:</span>
                      <span className="text-sm">{exam.grade}</span>
                    </div>
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Duração:</span>
                      <span className="text-sm">{exam.duration} minutos</span>
                    </div>
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Pontuação total:</span>
                      <span className="text-sm">{exam.totalPoints} pontos</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Criada em:</span>
                      <span className="text-sm">{exam.createdAt}</span>
                    </div>
                    <div className="flex items-center">
                      <span className="w-32 text-sm text-muted-foreground">Atualizada em:</span>
                      <span className="text-sm">{exam.updatedAt}</span>
                    </div>
                    {exam.scheduledFor && (
                      <div className="flex items-center">
                        <span className="w-32 text-sm text-muted-foreground">Agendada para:</span>
                        <span className="text-sm">{exam.scheduledFor}</span>
                      </div>
                    )}
                    {exam.responseCount && (
                      <div className="flex items-center">
                        <span className="w-32 text-sm text-muted-foreground">Respostas:</span>
            ...
            <Tabs defaultValue="questions" className="space-y-4">
              <TabsList>
                <TabsTrigger value="questions">Questões ({exam.questions.length})</TabsTrigger>
                <TabsTrigger value="students">Alunos ({exam.students?.length || 0})</TabsTrigger>
                ...
                <Card>
                  <CardContent className="pt-6">
                    {exam.questions.map((question, index) => (
                      <div key={question.id} className="mb-6 pb-6 border-b last:border-0 last:mb-0 last:pb-0">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3 className="font-medium">Questão {index + 1}</h3>
                            <div className="flex gap-2 mt-1">
                              <Badge variant="outline">{getQuestionTypeLabel(question.type)}</Badge>
                              <Badge variant="outline">{question.points} {question.points === 1 ? 'ponto' : 'pontos'}</Badge>
                            </div>
                          </div>
                        </div>
                        ...
                        {question.type === "essay" && (
                          <div className="pl-4 mt-2">
                            <p className="text-sm text-muted-foreground italic">Questão dissertativa - correção manual ou assistida por IA</p>
                          </div>
                        )}
                      </div>
                    ))}
                        ...
                        <tbody>
                          {exam.students?.map((student) => (
                            <tr key={student.id} className="border-b last:border-0 hover:bg-muted/50">
                              ...
                              <td className="py-2 px-2">{renderStudentStatusBadge(student.status)}</td>
                              <td className="py-2 px-2">
                                {student.score !== undefined && student.maxScore !== undefined ? (
                                  <span className={
                                    student.score / student.maxScore >= 0.7 ? "text-green-600" :
                                    student.score / student.maxScore >= 0.5 ? "text-yellow-600" :
                                    "text-red-600"
                                  }>
                              ...
                              <td className="py-2 px-2">
                                {student.submittedAt || <span className="text-muted-foreground">-</span>}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{exam.averageScore?.toFixed(1)}/{exam.totalPoints}</span>
                            </div>
                            <Progress value={(exam.averageScore || 0) / exam.totalPoints * 100} className="h-2" />
                          </div>
                          
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              <span>Taxa de Conclusão</span>
                              <span className="font-medium">{exam.completionRate}%</span>
                            </div>
                            <Progress value={exam.completionRate || 0} className="h-2" />
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-6">
                            ...
                            <div className="bg-muted rounded-lg p-4 text-center">
                              <div className="text-3xl font-bold text-yellow-600">
                                {exam.students?.filter(s => s.status === "in_progress").length || 0}
                              </div>
                              <div className="text-sm text-muted-foreground">Em andamento</div>
                            </div>
                            <div className="bg-muted rounded-lg p-4 text-center">
                              <div className="text-3xl font-bold text-blue-600">
                                {exam.students?.filter(s => s.status === "pending").length || 0}
                              </div>
                              <div className="text-sm text-muted-foreground">Pendentes</div>
                            </div>
                            <div className="bg-muted rounded-lg p-4 text-center">
                              <div className="text-3xl font-bold text-red-600">
                                {exam.students?.filter(s => s.status === "absent").length || 0}
                              </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{exam.duration} minutos</p>
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{exam.scheduledFor || "Não agendada"}</p>
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{exam.students?.length || 0} alunos</p>
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{exam.averageScore?.toFixed(1) || "-"}/{exam.totalPoints}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
      ...
      <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button onClick={() => setShowShareDialog(false)} variant="outline">Cancelar</Button>
            <Button>
